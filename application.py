...
def index():
    ...
    stocks_count = db.execute("SELECT SUM(shares), ticker, company_name FROM purchases WHERE id = ? GROUP BY ticker, company_name HAVING SUM(shares) > 0;", (session["user_id"])) # URGENT take out the ", company_name"
    print(stocks_count)
